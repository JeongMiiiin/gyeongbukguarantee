let reserveManageListUpdate = {
	...
	,init : function(){
		...
		this.selectTimeCon = $('#rlReserveTime');
		...
		if($('#rlReserveDate').val() != ''){
			
			let targetDate = $('#rlReserveDate').val();  
			
			this.initSettingTimeValue = $('[name="prevRlReserveTime"]').val(); 
	...
	,registEvent : function(){
		...
		$('[name="brIdx"]').on('change', function(){
			if(_this.brIdx != $(this).val() && $(this).val() != ''){
				_this.brIdx = $(this).val();
				_this.getBranchData();
				$('[name="rlReserveDate"]').val('');
				$('[name="brIdx"]').find('option[value=""]').remove();
				_this.makeCalendar();
				$('[name="rlReserveTime"]').empty();
			}
		})
		
		/*$('[data-common-datepicker]').on('click', function(){
